...
export function handleChargeFundingRate(event: ChargeFundingRateEvent): void {
  for (let i = 0, lenQ = event.params.quoteIds.length; i < lenQ; i++) {
    ...
    if (entity.positionType) {
      // SHORT position
      entity.openedPrice = entity.openedPrice!.minus(
        entity.openedPrice!.times(event.params.rates[i]).div(FACTOR)
      );
    } else {
      entity.openedPrice = entity.openedPrice!.plus(
        entity.openedPrice!.times(event.params.rates[i]).div(FACTOR)
      );
    }
  ...
  if (partyAEntity) {
    ...
    for (let i = 0, lenQ = list.length; i < lenQ; i++) {
      ...
      if (pendingEntity.quoteStatus <= 2 && pendingEntity.quoteStatus >= 0) {
        ...
        log.error(
          `error in liquidate positions party A\nQuoteId: ${quoteId}\nQuote status: ${pendingEntity.quoteStatus}`,
          []
        );
      }
    }

    let liquidTrEntity = new LiquidTransaction(
    ...
    const balance = allocatedBalanceOfPartyA(
      event.params.partyA,
      event.address
    );
    if (balance) {
    ...
    for (let i = 0, lenQ = list.length; i < lenQ; i++) {
      ...
      if (pendingEntity.quoteStatus <= 2 && pendingEntity.quoteStatus >= 0) {
        ...
        if (pendingEntity.partyB) {
          let partyAPartyBEntity = PartyApartyB.load(
            event.params.partyA.toHexString() +
              "-" +
              pendingEntity.partyB!.toHexString()
          )!;
...
export function handleLiquidatePartyB(event: LiquidatePartyBEvent): void {
  let partyAPartyBEntity = PartyApartyB.load(
    event.params.partyA.toHexString() + "-" + event.params.partyB.toHexString()
  )!;
  ...
  if (partyAPartyBEntity) {
    for (let i = 0, lenQ = list.length; i < lenQ; i++) {
      ...
      if (entity.quoteStatus <= 2 && entity.quoteStatus >= 0) {
        ...
        log.error(
          `error in liquidate positions party B\nQuoteId: ${quoteId}\nQuote status: ${entity.quoteStatus}`,
          []
        );
      }
    }
    ...
    const balance = allocatedBalanceOfPartyB(
      event.params.partyB,
      event.params.partyA,
      event.address
    );
  ...
  for (let i = 0, lenQ = event.params.quoteIds.length; i < lenQ; i++) {
    ...
    if (callResult.reverted) {
      ...
      if (getAveragePrice.gt(BigInt.fromI32(0))) {
        entity.liquidatePrice = getAveragePrice
          .times(getclosedAmount)
          .minus(entity.averageClosedPrice!.times(entity.closedAmount!))
          .div(LiquidateAmount);
      } else {
        log.debug(
          `get total fill amount: ${getclosedAmount} , past total fill amount: ${entity.closedAmount!.toString()}\nQuoteId: ${
            entity.quoteId
          }`,
          []
        );
...
export function handleExpireQuote(event: ExpireQuoteEvent): void {
  ...
  if (entity.quoteStatus === 2) {
    log.debug(
      `Quote id: ${entity.quoteId} , expire tr hash: ${event.transaction.hash}`,
      []
    );
  }
  entity.quoteId = event.params.quoteId;
  ...
  if (entity.quoteStatus === 0) {
    ...
    let partyAPartyBEntity = PartyApartyB.load(
      entity.partyA.toHexString() + "-" + entity.partyB!.toHexString()
    )!;
...
export function handleForceClosePosition(event: ForceClosePositionEvent): void {
  ...
  entity.averageClosedPrice = entity
    .closedAmount!.times(entity.averageClosedPrice!)
    .plus(event.params.filledAmount.times(event.params.closedPrice))
    .div(entity.closedAmount!.plus(event.params.filledAmount));
...
export function handleSendQuote(event: SendQuoteEvent): void {
  ...
  let callResult = symmioContract.try_symbolNameByQuoteId([
...
export function handleAcceptCancelRequest(
  ...
  if (entity) {
    ...
    newEntity.timestampsAcceptCancelCloseRequestTimeStamp =
    ...
    if (callResult.reverted) {
      ...
      let initialNewEntity = initialHelper(Result);
      if (initialNewEntity) {
        const symbol = symbolIdToSymbolName(
          initialNewEntity.symbolId,
          event.address
        );
        if (symbol) {
      ...
      newEntity.tradingFee = new BigInt(0);
    }
    newEntity.save();
  }
}
...
export function handleLockQuote(event: LockQuoteEvent): void {
  ...
  if (entity) {
    ...
    entity.save();
    let partyAPartyBEntity = PartyApartyB.load(
      entity.partyA.toHexString() + "-" + event.params.partyB.toHexString()
    );
    if (!partyAPartyBEntity) {
      partyAPartyBEntity = new PartyApartyB(
        entity.partyA.toHexString() + "-" + event.params.partyB.toHexString()
      );
