...
import { DebugEntity, ResultEntity, GlobalFee } from "../generated/schema";
...
export function handleChargeFundingRate(event: ChargeFundingRateEvent): void {
  for (let i = 0, lenQ = event.params.quoteIds.length; i < lenQ; i++) {
    ...
    let newPrice: BigInt;
    if (entity.positionType === 0) {
      //Long
      newPrice = entity.openedPrice!.plus(
        entity.openedPrice!.times(rate).div(FACTOR)
      );
    } else {
      newPrice = entity.openedPrice!.minus(
        entity.openedPrice!.times(rate).div(FACTOR)
      );
    }
    let fee: BigInt;
    fee = entity
      .openedPrice!.times(rate)
